...
contract GeneScienceInterface {
    ...
    /// @dev given genes of monsten 1 & 2, return a genetic combination - may have a random factor
...
contract MonsterAccessControl {
    // This facet controls access control for monster. There are four roles managed here:
    //
...
/// @title Base contract for monster. Holds all common structs, events and base variables.
/// @author Axiom Zen (https://www.axiomzen.co)
/// @dev See the MonsterCore contract documentation to understand how the various contract facets are arranged.
contract MonsterBase is MonsterAccessControl {
    /*** EVENTS ***/

    /// @dev The Birth event is fired whenever a new monsten comes into existence. This obviously
    ...
    event Birth(address owner, uint256 MonsterId, uint256 matronId, uint256 sireId, uint256 genes);

    /// @dev Transfer event as defined in current draft of ERC721. Emitted every time a monsten
    ///  ownership is assigned, including births.
    event Transfer(address from, address to, uint256 tokenId);

    /*** DATA TYPES ***/

    /// @dev The main Monster struct. Every monster is represented by a copy
    ...
    /// @dev An array containing the Monster struct for all Kitties in existence. The ID
    ///  of each monster is actually an index into this array. Note that ID 0 is a negacat,
        ...
        returns (uint)
    {
        ...
        uint256 newMonstenId = monsters.push(_monster) - 1;

        // It's probably never going to happen, 4 billion monsters is A LOT, but
        // let's just be 100% sure we never let this happen.
        require(newMonstenId == uint256(uint32(newMonstenId)));
        ...
        // per ERC721 draft
        _transfer(0, _owner, newMonstenId);
...
contract MonsterOwnership is MonsterBase, ERC721 {
    ...
    string public constant symbol = "MR";
    ...
    /// @param _tokenId monsten id, only valid when > 0
    ...
    /// @param _claimant the address we are confirming monsten is approved for.
    /// @param _tokenId monsten id, only valid when > 0
    ...
    ///  monster specifically) or your Monster may be lost forever. Seriously.
    ...
    function tokensOfOwner(address _owner) external view returns(uint256[] ownerTokens) {
        ...
        if (tokenCount == 0) {
            ...
            for (monsterId = 1; monsterId <= totalCats; monsterId++) {
...
contract MonsterBreeding is MonsterOwnership {
    ...
    /// @dev Checks that a given monsten is able to breed. Requires that the
    ///  current cooldown is finished (for sires) and also checks that there is
    ///  no pending pregnancy.
    function _isReadyToBreed(Monster _mon) internal view returns (bool) {
        // In addition to checking the cooldownEndBlock, we also need to check to see if
        // the monster has a pending birth; there can be some period of time between the end
        // of the pregnacy timer and the birth event.
        return (_mon.siringWithId == 0) && (_mon.cooldownEndBlock <= uint64(block.number));
    }
    ...
    /// @param _monsten A reference to the Monster in storage which needs its timer started.
    function _triggerCooldown(Monster storage _monsten) internal {
        // Compute an estimation of the cooldown time in blocks (based on current cooldownIndex).
        _monsten.cooldownEndBlock = uint64((cooldowns[_monsten.cooldownIndex]/secondsPerBlock) + block.number);

        // Increment the breeding count, clamping it at 13, which is the length of the
        // cooldowns array. We could check the array size dynamically, but hard-coding
        // this as a constant saves gas. Yay, Solidity!
        if (_monsten.cooldownIndex < 13) {
            _monsten.cooldownIndex += 1;
        }
    }
    ...
    /// @notice Checks that a given monsten is able to breed (i.e. it is not pregnant or
    ///  in the middle of a siring cooldown).
    /// @param _monsterId reference the id of the monsten, any user can inquire about it
        ...
        returns (bool)
    {
        ...
        Monster storage mon = monsters[_monsterId];
        return _isReadyToBreed(mon);
    }
    ...
    /// @return The Monster ID of the new monsten.
    /// @dev Looks at a given Monster and, if pregnant and if the gestation period has passed,
    ///  combines the genes of the two parents to create a new monsten. The new Monster is assigned
    ///  to the current owner of the matron. Upon successful completion, both the matron and the
    ///  new monsten will be ready to breed again. Note that anyone can call this function (if they
    ///  are willing to pay the gas!), but the new monsten always goes to the mother's owner.
        ...
        returns(uint256)
    {
        ...
        // Make the new monsten!
        address owner = monsterIndexToOwner[_matronId];
        uint256 monstenId = _createMonster(_matronId, matron.siringWithId, parentGen + 1, childGenes, owner);
...
contract MonsterMinting is MonsterAuction {
    ...
    /// @dev we can create promo monstens, up to a limit. Only callable by COO
    /// @param _genes the encoded genes of the monsten to be created, any value is accepted
    /// @param _owner the future owner of the created monstens. Default to contract COO
    ...
    function createGen0Auction(uint256 _genes) external onlyCOO {
        ...
        uint256 monsterId = _createMonster(0, 0, 0, _genes, address(this));
...
/// @title monster: Collectible, breedable, and oh-so-adorable monsters on the Ethereum blockchain.
/// @author Axiom Zen (https://www.axiomzen.co)
/// @dev The main monster contract, keeps track of monstens so they don't wander around and get lost.
contract MonsterCore is MonsterMinting {

    // This is the main monster contract. In order to keep our code seperated into logical sections,
    ...
    /// @notice Creates the main monster smart contract instance.
    function MonsterCore() public {
        ...
        // start with the mythical monsten 0 - so we don't have generation-0 parent issues
    ...
    function getMonster(uint256 _id)
        ...
        isGestating = (mon.siringWithId != 0);
        isReady = (mon.cooldownEndBlock <= block.number);
        cooldownIndex = uint256(mon.cooldownIndex);
        nextActionAt = uint256(mon.cooldownEndBlock);
        siringWithId = uint256(mon.siringWithId);
        birthTime = uint256(mon.birthTime);
        matronId = uint256(mon.matronId);
        sireId = uint256(mon.sireId);
    ...
    function withdrawBalance() external onlyCFO {
        ...
        // Subtract all the currently pregnant monstens we have, plus 1 of margin.
